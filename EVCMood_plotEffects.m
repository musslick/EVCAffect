function EVCMood_plotEffects(filePath, plotRT)
...
disp(['mean accuracy (lower parameter range): ' num2str(1-mean(ER_results(1).Gratton.overallPerformance))]);
if(length(ER_results) > 1)
    disp(['mean accuracy (upper parameter range): ' num2str(1-mean(ER_results(2).Gratton.overallPerformance))]);
end
...
%     title(titles{paramIdx}, 'FontSize', fontSize.title, 'FontWeight', fontWeight.title);
    ...
    fig3 = figure(3);
    set(fig3, 'Position', [600 450 450 200]);
