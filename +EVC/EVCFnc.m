classdef EVCFnc < handle
    ...
    properties (Constant)
        ...
        paramReq = [1, ... VALUE:       f(x) = p1
                    ...
                    4, ... REWRATE_LOGVAL:     f(x) = sum(P(outcome) * (p3 * log(V(outcome)*p4))/ (p1 + p2 * reactionTime(outcome)), outcomes)
                    ];   
    end
    ...
    methods (Access = public)
        ...
        function out = getVal(this, varargin)
            ...
            if(this.type == this.REWRATE_LOGVAL)
                out = getRewardRateLogValue(this, inputVal);
            end
...
%         end
        ...
        % sum values over all outcomes
        % inputVal ...current state
        function out = getRewardRateLogValue(this, inputVal)
            ...
            % pre-specified trial outcomes
            if(this.EVCModel.useActualState)
                values = this.params{3} * log(this.EVCModel.State.Actual.outcomeValues *  this.params{4} + 1);
            else
                values = this.params{3} * log(this.EVCModel.State.Expected.outcomeValues *  this.params{4} + 1);
            end
